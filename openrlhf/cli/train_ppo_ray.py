...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_start_steps", type=int, default=50)
    parser.add_argument("--save_steps", type=int, default=-1)
    parser.add_argument("--logging_steps", type=int, default=1)
    parser.add_argument("--diversity_score_steps", type=int, default=1)
    ...
    parser.add_argument("--stop_at_step_200", action="store_true", default=False, help="Stop training at step 200 if the HF checkpoint for step 200 exists.")
    ...
    parser.add_argument("--actor_loss_coef", type=float, default=1, help="Actor loss coefficient")
    parser.add_argument("--aux_loss_coef", type=float, default=0, help="MoE balancing loss")
    parser.add_argument("--sftchat_loss_coef", type=float, default=0, help="SFT chatability loss")
    parser.add_argument("--postfill_cot_loss_coef", type=float, default=0, help="Postfill COT loss")
    parser.add_argument("--sft_steps", type=int, default=1, help="Train SFT component every N global steps.")
    parser.add_argument("--sft_batches_per_step", type=int, default=1, help="Number of SFT mini-batches per SFT training step.")
    parser.add_argument("--adam_betas", type=float, nargs=2, default=(0.9, 0.95), help="Betas for Adam optimizer")
    parser.add_argument("--reward_clip_range", type=float, nargs=2, default=(-10, 10), help="Reward clip range")

    # multi-turn self-play
    parser.add_argument("--taboo_game", action="store_true", default=False, help="Enable taboo game")
    parser.add_argument("--custom_configs",
        type=json.loads,  # This will parse the JSON string into a dictionary
        default=None,
        help="Configuration for multi-turn conversation format. Must be a valid JSON string, e.g., '{\"max_turns\": 3}'"
    )
    ...
    parser.add_argument("--sft_input_key", type=str, default="input", help="JSON dataset key")
    parser.add_argument("--sft_output_key", type=str, default=None, help="JSON dataset key")
    ...
    parser.add_argument("--wandb_max_log", type=int, default=10)
    parser.add_argument("--wandb_table_log_interval", type=int, default=20)
    parser.add_argument("--wandb_table_csv_path", type=str, default=None)
    ...
    # Verify custom_configs keys spelling to avoid downstream errors
    expected_keys = ["max_turns", "reward_type", "remove_ties", "direct_chat_no_cot", "no_attacker_turn", "no_defender_turn"]
    if args.custom_configs:
        for key in args.custom_configs:
            if key not in expected_keys:
                print(f"[Warning] Unexpected key in custom_configs: '{key}'. Expected keys are: {expected_keys}")

    if args.custom_configs and args.custom_configs.get("direct_chat_no_cot", False):
        assert args.postfill_cot_loss_coef == 0.0, "postfill_cot_loss_coef should be 0.0 when direct_chat_no_cot is enabled"
